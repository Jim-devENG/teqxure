...
import { postDirectMessageAction, type ActionState } from "@/lib/actions/messages";
...
export function ThreadView({
  ...
  const [state, formAction] = useActionState(postDirectMessageAction.bind(null, threadId), initialState);
  const formRef = useRef<HTMLFormElement>(null);
  const bottomRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    const interval = setInterval(async () => {
      const res = await fetch(`/api/platform/messages/${threadId}`);
      if (!res.ok) return;

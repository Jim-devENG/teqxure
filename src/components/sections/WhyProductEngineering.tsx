        ...
        <div className="grid grid-cols-1 gap-16 lg:grid-cols-[1fr_0.85fr] lg:gap-12">
          <div>
            ...
            <div className="mt-12 grid grid-cols-1 gap-6 sm:grid-cols-2">
              <Reveal delay={0.15}>
                <div className="rounded-2xl border border-charcoal/10 bg-charcoal-soft/[0.02] p-6">
                  <p className="font-mono text-[11px] uppercase tracking-[0.15em] text-charcoal/40">
                    Tutorials
                  </p>
                  <ul className="mt-4 flex flex-col gap-3">
                    {TUTORIAL_TRAITS.map((trait) => (
                      <li key={trait} className="flex items-start gap-2.5 text-sm text-charcoal/60">
                        <X className="mt-0.5 h-4 w-4 shrink-0 text-charcoal/30" strokeWidth={1.5} />
                        {trait}
                      </li>
                    ))}
                  </ul>
                ...
                <div className="rounded-2xl border border-blue/20 bg-blue/[0.03] p-6">
                  ...
                  <ul className="mt-4 flex flex-col gap-3">
                    {PRODUCT_TRAITS.map((trait) => (
                      <li key={trait} className="flex items-start gap-2.5 text-sm text-charcoal/75">
          ...
          <Reveal delay={0.2} className="relative">
            ...
            <div className="panel-glass-light absolute -bottom-6 -right-6 max-w-[220px] rounded-xl px-5 py-4 shadow-xl">
              <p className="text-sm font-medium text-charcoal">Built in cohort teams</p>

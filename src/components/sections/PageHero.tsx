...
type Accent = "blue" | "cyan" | "emerald" | "slate";
...
const accentBg: Record<Accent, string> = {
  ...
  cyan: "bg-cyan hover:bg-cyan/85",
  emerald: "bg-emerald hover:bg-emerald/85",
  slate: "bg-slate hover:bg-slate/85",
};

interface PageHeroProps {
  sectionKey: "STUDIO_HERO" | "COMMUNITY_HERO" | "RESEARCH_HERO";
  accent: Accent;
}

export async function PageHero({ sectionKey, accent }: PageHeroProps) {
  ...
  return (
    <section className="relative overflow-hidden bg-charcoal pb-24 pt-36 sm:pt-44">
      ...
      <div className="relative mx-auto max-w-4xl px-6 text-center">
        <Reveal>
          <span className={cn("font-mono text-xs uppercase tracking-[0.2em]", accentText[accent])}>
            {section.pageTitle}
          </span>
        </Reveal>
        ...
        {section.ctaText && section.ctaUrl && (
          <Reveal delay={0.45}>
            <Link
              href={section.ctaUrl}
              className={cn(
                "mt-9 inline-flex items-center gap-2 rounded-full px-6 py-3 text-sm font-medium text-white transition-colors",
                accentBg[accent],
              )}
            >
              {section.ctaText}

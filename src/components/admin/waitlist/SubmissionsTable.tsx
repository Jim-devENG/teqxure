...
import { markSubmissionReviewedAction } from "@/lib/actions/waitlistSubmissions";
...
export function SubmissionsTable({ submissions }: { submissions: SubmissionRow[] }) {
  ...
  function markReviewed(id: string) {
    ...
    startTransition(() => {
      markSubmissionReviewedAction(id);
    });
  }

  if (rows.length === 0) {
    return <p className="text-sm text-slate">No submissions yet.</p>;
  }
